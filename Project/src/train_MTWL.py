...
def meanIoU(y_true_all, y_pred_all, num_classes=20):
    ...
    for j in range(num_examples):
        ...
        for i in clas:
            ...
            mIoU_class += ((intersection)/(union))

            # print("class: {} mIoU: {}".format(i, ((intersection + 1.0)/(union + 1.0))))
        
        mIoU += (mIoU_class / len(clas))
        # print("Example: {} mIoU: {}".format(j, mIoU_class / len(np.unique(y_true))))
...
def main(config):
    ...
    model = DeepLab(backbone='resnet', output_stride=8, num_classes=[20,1], sync_bn=False, freeze_bn=False)
    ...
    print("===> Initial Reg Weight and Seg weight: {}  {}".format(torch.exp(-reg_task_weight), torch.exp(-seg_task_weight)))
    print("===> Initial Reg Weight and Seg weight Require Grad: {}  {}".format(reg_task_weight.requires_grad, seg_task_weight.requires_grad))

    params = ([p for p in model.parameters()] + [reg_task_weight] + [seg_task_weight])
    ...
    training_data_loader = torch.utils.data.DataLoader( Cityscapes(root='/usr/local/data/raghav/ECSE626_2019/Project/data/', split='train', mode='fine', target_type=['semantic', 'depth'], transform=input_transform, target_transform=[label_transform, depth_transform] ), 
                                                        batch_size=8, 
                                                        shuffle=True, 
                                                        num_workers=4, 
                                                        drop_last=True)

    validation_data_loader = torch.utils.data.DataLoader( Cityscapes(root='/usr/local/data/raghav/ECSE626_2019/Project/data/', split='val', mode='fine', target_type=['semantic', 'depth'], transform=input_transform, target_transform=[label_transform, depth_transform] ), 
                                                        batch_size=8, 
                                                        shuffle=True, 
                                                        num_workers=4, 
                                                        drop_last=True)
    ...
    my_loss = ["loss", "loss_seg", "loss_reg", "reg_weight", "seg_weight", "reg_precision", "seg_precision"]
    ...
    def checkpoint(epoch):
        ...
        torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'seg_task_weight': seg_task_weight, 'reg_task_weight': reg_task_weight}, join(log_path, w_path))
    ...
    def train(epoch):
        ...
        for iteration, batch in enumerate(tqdm(training_data_loader)):
            ...
            inp, target, seg = batch[0].to(device), batch[1][1].to(device), batch[1][0].type('torch.LongTensor').squeeze().to(device)
            ...
            loss_seg = (loss_seg*seg_precision[0]) + seg_task_weight[0]
            loss_seg = torch.mean(loss_seg)

            reg_precision = torch.exp(-reg_task_weight)
            loss_reg =  reg_crit(outp[1], target)
            loss_reg = (loss_reg*reg_precision[0]) + reg_task_weight[0]
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            for iteration, batch in enumerate(training_data_loader):
                ...
                metric += np.array([loss, loss_seg, loss_reg, reg_task_weight.item(), seg_task_weight.item(), reg_precision.item(), seg_precision.item(), mIoU, mean_squared_error, mean_absolute_error])
    ...
    def visualize(epch):
        ...
        with torch.no_grad():
            ...
            for iteration, batch in enumerate(validation_data_loader):
                ...
                outp_reg = np.squeeze(outp[1].data.cpu().numpy().astype('float32'))

                ####
                for i in range(inp.shape[0]):
                    result = Image.fromarray(seg[i,...].squeeze().astype(np.uint8))
                    result.save(join(v_path, 'seg{}.png'.format(i)))
                    result = Image.fromarray(outp_seg[i,...].squeeze().astype(np.uint8))
                    result.save(join(v_path, 'outp_seg{}.png'.format(i)))
                    result = Image.fromarray(255*target[i,...].squeeze().astype(np.uint8))
                    result.save(join(v_path, 'depth{}.png'.format(i)))
                    result = Image.fromarray(255*outp_reg[i,...].squeeze().astype(np.uint8))
    ...
    for epch in range(initial_epoch, num_epochs):
        ...
        if epch % 2 == 0:
            ...
            train_metric = test()
            print("===> Training   Epoch {}: Loss - {:.4f}, Loss_Seg - {:.4f}, Loss_Reg - {:.4f}, mean IoU - {:.4f}, MSE - {:.4f}, MAE - {:.4f}".format(epch, train_metric[0], train_metric[1], train_metric[2], train_metric[7], train_metric[8], train_metric[9]))

            val_metric = validate()
            print("===> Validation Epoch {}: Loss - {:.4f}, Loss_Seg - {:.4f}, Loss_Reg - {:.4f}, mean IoU - {:.4f}, MSE - {:.4f}, MAE - {:.4f}".format(epch, val_metric[0], val_metric[1], val_metric[2], val_metric[7], val_metric[8], val_metric[9]))

            print("===> Loss weights Epoch {}: Reg_weight - {:.4f}, Seg_weight - {:.4f}".format(epch, val_metric[3], val_metric[4]))
            print("===> Loss precision Epoch {}: reg_precision - {:.4f}, seg_precision - {:.4f}".format(epch, val_metric[5], val_metric[6]))
            print("===> Loss = (reg_precision*Loss_Reg) + (seg_precision*Loss_Seg) + Reg_weight + Seg_weight")
