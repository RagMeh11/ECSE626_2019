...
def main(config):
    ...
    model = DeepLab(backbone='resnet', output_stride=8, num_classes=[1], sync_bn=False, freeze_bn=False)
    ...
    training_data_loader = torch.utils.data.DataLoader( Cityscapes(root='/usr/local/data/raghav/ECSE626_2019/Project/data/', split='train', mode='fine', target_type=['depth'], transform=input_transform, target_transform=[depth_transform] ), 
                                                        batch_size=8, 
                                                        shuffle=True, 
                                                        num_workers=4, 
                                                        drop_last=True)

    validation_data_loader = torch.utils.data.DataLoader( Cityscapes(root='/usr/local/data/raghav/ECSE626_2019/Project/data/', split='val', mode='fine', target_type=['depth'], transform=input_transform, target_transform=[depth_transform] ), 
                                                        batch_size=8, 
                                                        shuffle=True, 
                                                        num_workers=4, 
                                                        drop_last=True)
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            for iteration, batch in enumerate(training_data_loader):
                ...
                inp, target = batch[0].to(device),  batch[1].to(device) #batch[1][1].to(device), target,
                ...
                mean_absolute_error = torch.mean(torch.abs(outp[0]-target)).item()

                metric += np.array([loss, mean_squared_error, mean_absolute_error])
    ...
    def visualize(epch):
        ...
        with torch.no_grad():
            ...
            for iteration, batch in enumerate(validation_data_loader):
                ...
                outp_reg = np.squeeze(outp[0].data.cpu().numpy().astype('float32'))
    ...
    for epch in range(initial_epoch, num_epochs):
        ...
        if epch % 2 == 0:
            ...
            train_metric = test()
            print("===> Training   Epoch {}: Loss - {:.4f}, MSE - {:.4f}, MAE - {:.4f}".format(epch, train_metric[0], train_metric[1], train_metric[2]))

            val_metric = validate()
            print("===> Validation Epoch {}: Loss - {:.4f}, MSE - {:.4f}, MAE - {:.4f}".format(epch, val_metric[0], val_metric[1], val_metric[2]))
