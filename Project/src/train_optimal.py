...
def main(config):
    ...
    ConfigName = 'Seg_Depth_optimal_1'                                                     # Configuration Name to Uniquely Identify this Experiment
    ...
    my_loss = ["loss", "loss_seg", "loss_reg"]
    ...
    def train(epoch):
        ...
        for iteration, batch in enumerate(tqdm(training_data_loader)):
            ...
            loss = (0.2*loss_seg) + (0.8*loss_reg)
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            for iteration, batch in enumerate(training_data_loader):
                ...
                metric += np.array([loss, loss_seg, loss_reg, mIoU, mean_squared_error, mean_absolute_error])
    ...
    for epch in range(initial_epoch, num_epochs):
        ...
        if epch % 2 == 0:
            ...
            train_metric = test()
            print("===> Training   Epoch {}: Loss - {:.4f}, Loss_Seg - {:.4f}, Loss_Reg - {:.4f}, mean IoU - {:.4f}, MSE - {:.4f}, MAE - {:.4f}".format(epch, train_metric[0], train_metric[1], train_metric[2], train_metric[3], train_metric[4], train_metric[5]))
